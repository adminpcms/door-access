...
def updateCard(cardID,excelLineNo):
	...
	rangeNames = ['Sheet1!A2:E','Sheet1!F2:F','Sheet1!K2:K']
	#print(service.spreadsheets().values().batchGet(spreadsheetId=spreadsheetId, ranges=rangeNames, valueRenderOption='UNFORMATTED_VALUE', dateTimeRenderOption='SERIAL_NUMBER').execute())
	service.spreadsheets().values().update(spreadsheetId=spreadsheetId,range='Sheet1!K'+str(excelLineNo),valueInputOption="RAW",body={'values' : [[cardID]]}).execute()
